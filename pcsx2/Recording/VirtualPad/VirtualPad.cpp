...
const wxSize VirtualPad::s_SPINNER_SIZE = wxSize(100, 40);
	...
	, options(options)
{
	...
	const float dpiScale = MSW_GetDPIScale();                // linux returns 1.0
	...
	SetClientSize(m_virtualPadData.m_background.image.GetWidth(), m_virtualPadData.m_background.image.GetHeight());
...
void VirtualPad::Render(wxDC& bdc)
{
	...
	if (!m_manualRedrawMode || m_clearScreenRequired || !m_renderQueue.empty())
	{
		...
		bdc.DrawBitmap(m_virtualPadData.m_background.image, m_virtualPadData.m_background.coords, true);
		...
		{
			m_renderQueue.front()->Render(bdc);
			m_renderQueue.pop();
		}
	}
}
...
{
	const wxPoint scaledPoint = ScaledPoint(pressureSpinnerCoord, s_SPINNER_SIZE, rightAlignedCoord, bottomAlignedCoord);

	button.m_icon = image;
	button.m_pressureSpinner = new wxSpinCtrl(parentWindow, wxID_ANY, wxEmptyString, scaledPoint, ScaledSize(s_SPINNER_SIZE), wxSP_ARROW_KEYS, 0, 255, 0);
...
{
	const wxPoint xSpinnerScaledPoint = ScaledPoint(xSpinnerPoint, s_SPINNER_SIZE, rightAlignedSpinners);
	const wxPoint ySpinnerScaledPoint = ScaledPoint(ySpinnerPoint, s_SPINNER_SIZE, rightAlignedSpinners, true);
	...
	wxSpinCtrl* xSpinner = new wxSpinCtrl(parentWindow, wxID_ANY, wxEmptyString, xSpinnerScaledPoint, ScaledSize(s_SPINNER_SIZE), wxSP_ARROW_KEYS, 0, 255, 127);
	wxSpinCtrl* ySpinner = new wxSpinCtrl(parentWindow, wxID_ANY, wxEmptyString, ySpinnerScaledPoint, ScaledSize(s_SPINNER_SIZE), wxSP_ARROW_KEYS, 0, 255, 127);
	...
	analog.m_positionGraphic.endCoords
		= analog.m_positionGraphic.centerCoords
		= ScaledPoint(centerPoint);
	analog.m_positionGraphic.radius = radius * m_scalingFactor;
	analog.m_positionGraphic.lineThickness = 6 * m_scalingFactor;
