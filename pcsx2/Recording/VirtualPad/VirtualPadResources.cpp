...
void ControllerNormalButton::UpdateGuiElement(std::queue<VirtualPadElement*>& renderQueue, bool& clearScreenRequired)
{
	...
	// We only render the button if it is m_pressed
	if (m_pressed)
		renderQueue.push(this);
	// However, if the button has been drawn to the screen in the past
	// we need to ensure the screen is cleared.
	// This is needed in the scenario where only a single button is being m_pressed/released
...
{
	dc.DrawBitmap(m_icon.image, m_icon.coords, true);
...
void AnalogStick::Render(wxDC& dc)
{
	...
	int newXCoord = m_positionGraphic.centerCoords.x + ((m_xVector.m_val - 127) / 127.0) * m_positionGraphic.radius;
	int newYCoord = m_positionGraphic.centerCoords.y + ((m_yVector.m_val - 127) / 127.0) * m_positionGraphic.radius;
	...
	const float lengthOfLine = sqrt(pow(newXCoord - m_positionGraphic.centerCoords.x, 2) + pow(newYCoord - m_positionGraphic.centerCoords.y, 2));
	if (lengthOfLine > m_positionGraphic.radius)
	{
		newXCoord = ((1 - m_positionGraphic.radius / lengthOfLine) * m_positionGraphic.centerCoords.x) + m_positionGraphic.radius / lengthOfLine * newXCoord;
		newYCoord = ((1 - m_positionGraphic.radius / lengthOfLine) * m_positionGraphic.centerCoords.y) + m_positionGraphic.radius / lengthOfLine * newYCoord;
	}
	// Set the new end coordinate
	m_positionGraphic.endCoords = wxPoint(newXCoord, newYCoord);
	// Draw line and tip
	dc.SetPen(wxPen(*wxBLUE, m_positionGraphic.lineThickness));
	dc.DrawLine(m_positionGraphic.centerCoords, m_positionGraphic.endCoords);
	dc.DrawCircle(m_positionGraphic.endCoords, wxCoord(m_positionGraphic.lineThickness));
	dc.SetPen(wxNullPen);
	m_currentlyRendered = true;
}
...
{
	return UpdateButtonData(padDataVal, ignoreRealController, readOnly);
}
...
bool ControllerButton::UpdateButtonData(bool& padDataVal, bool ignoreRealController, bool readOnly) noexcept
{
...
bool ControllerPressureButton::UpdateData(u8& padDataVal, bool ignoreRealController, bool readOnly) noexcept
{
	if (!ignoreRealController || readOnly)
	{
...
bool AnalogVector::UpdateData(u8& padDataVal, bool ignoreRealController, bool readOnly) noexcept
{
	if (!ignoreRealController || readOnly)
	{
