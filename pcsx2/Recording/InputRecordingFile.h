...
class InputRecordingFile
{
	...
	private:
		...
		static const int s_seekpointPads = s_seekpointRedoCount + 5; // Skips startType: seekpointRedoCount + 4
	};
...
public:
	...
	bool ReadKeyBuffer(u8& result, const s32 frame, const u32 seekOffset) const;
	// Writes the current frame's input data to the file so it can be replayed
	bool WriteKeyBuffer(const u8 buf, const s32 frame, const u32 seekOffset) const;
