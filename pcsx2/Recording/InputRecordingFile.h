...
class InputRecordingFile
{
	struct InputRecordingFileHeader
	{
		...
		bool ReadHeader(FILE* recordingFile);
	} m_header;
	...
	s64 getRecordingBlockSeekPoint(const s32 frame) const noexcept;
	bool open(const wxString path, const bool newRecording);
	bool verifyRecordingFileHeader();

public:
	static const s64 s_controllerInputBytes = 18;
	// Both of the below values will be made private in the future
	static const int s_seekpointInputData = InputRecordingFileHeader::s_seekpointRedoCount + 4;
	static const int s_inputBytesPerFrame = 2 * s_controllerInputBytes;
	...
	// booting the game
	bool OpenNew(const wxString& path, const bool fromSaveState);
	...
	void SetTotalFrames(const long frames) noexcept;
	...
	bool ReadKeyBuffer(u8& result, const s32 frame, const uint port, const uint bufIndex) const;
	// Writes the current frame's input data to the file so it can be replayed
	bool WriteKeyBuffer(const u8 buf, const s32 frame, const uint port, const uint bufIndex) const;
};
