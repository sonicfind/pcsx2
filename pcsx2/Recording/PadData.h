...
public:
	...
	void LogPadData(const u8 port);

private:
	// Checks and returns if button a is pressed or not
	bool IsButtonPressed(ButtonResolver buttonResolver, u8 const& bufVal) noexcept;
	u8 BitmaskOrZero(bool pressed, ButtonResolver buttonInfo) noexcept;
